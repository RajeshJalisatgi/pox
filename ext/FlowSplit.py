...
msg_ip_subnet1_inport.match.tp_src
""" 
...
def flowA(event):
    ...
    if len(pathB) > 1 :
        ...
        print("Shifting load from path A ",[tcp_src_port, tcp_dst_port])  
...
"""def update_stats(event):
    ...
    if port == 2
        ...
        print x =out.split(" ")[-1]
        if x > lam1
            print("Switching flow through h2") 
            port = 2
            flowA(event) """
...
class MyController (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    if packet.find("tcp"):
        ...
        pathA.append([tcp_src_port,tcp_dst_port])
        print "Handling Pac_handle_PacketInketIn"
        #pass IP on subnet 1 through one path
        msg_ip_subnet1_inport = of.ofp_flow_mod()
        msg_ip_subnet1_inport.match.dl_type = pkt.ethernet.IP_TYPE
        msg_ip_subnet1_inport.match.nw_proto = 0x06
        ...
        #print ("port=%d",msg_ip_icmp_inport.match.in_port)  
...
def launch ():
  ...
  def start_switch (event):
    ...
    inport = 1
    outport1 = 2
    print ("Start_Switch is executing")
    # pass ARP both ways
    ...
    #print("Controlling %s" % (event.connection,))
    #flowA(event)
    ...
    if event.dpid in splitting_table:
        inport, outport1, oProject_3a.pyutport2 = splitting_table[event.dpid]
